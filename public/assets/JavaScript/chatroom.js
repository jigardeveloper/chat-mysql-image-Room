...
document.querySelector("#msgfileForm").addEventListener("submit", sendMsgFile);
document.querySelector("#msgfile").addEventListener("change", onChangeMsgFile);
...
async function checkAccesskey() {
  ...
  // save user info
  const { id, display_name, avatar_dirct } = await fetch(
    `/api/users/${accesskey}`
  ).then((r) => r.json());
  userInfo.id = id;
...
async function roomList() {
  ...
  for (let i = 0; i < rooms.length; i++) {
    document.querySelector(
      "#roomList"
    ).innerHTML += `<li><button class="btn btn-color chatroomBtn btnChatRoomsize" id="room-${rooms[i].id}">${rooms[i].room_name}</button></li>`;
    document.querySelector(
      "#overlayRoomList"
    ).innerHTML += `<li><button class="btn btn-info chatroomBtn" id="overlayRoom-${rooms[i].id}">${rooms[i].room_name}</button>
    ...
    document.querySelector(
      "#sbRoomList"
    ).innerHTML += `<li class="center"><button class="btn btn-info chatroomBtn btnSize" id="sbRoom-${rooms[i].id}">${rooms[i].room_name}</button></li>`;
  }
  // add event listeners
  for (let i = 0; i < rooms.length; i++) {
    document
      .querySelector(`#room-${rooms[i].id}`)
      .addEventListener("click", () => {
        joinRoom(rooms[i]);
      });
    document
      .querySelector(`#overlayRoom-${rooms[i].id}`)
      .addEventListener("click", () => {
        joinRoom(rooms[i]);
      });
    document
      .querySelector(`#sbRoom-${rooms[i].id}`)
      .addEventListener("click", () => {
        joinRoom(rooms[i]);
      });
  }
...
function triggerModal(id) {
  document
    .querySelector("#deleteRoomBtn")
    .setAttribute("onClick", `delRoom(${id})`);
}
...
async function userList() {
  ...
  const users = await fetch(`/api/online/${currentRoomId}`).then((r) =>
    r.json()
  );
  ...
  for (let i = 0; i < users.length; i++) {
    document.querySelector(
      "#userList"
    ).innerHTML += `<li><img src="/assets/avatars/${users[i].avatar}" alt="avatar" height="25px" width="25px"/> ${users[i].displayName}</li>`;
    document.querySelector(
      "#sbUserList"
    ).innerHTML += `<li>${users[i].displayName}</li>`;
  }
}
...
async function prevMsgs(roomId) {
  ...
  const prev = await fetch(`/api/messages/${roomId}`)
    .then((r) => r.json())
  ...
  for (let i = 0; i < prev.length; i++) {
    document.querySelector(
      "#msgList"
    ).innerHTML += `<li><img src="/assets/avatars/${prev[i].avatar_dirct}" alt="avatar" height="25px" width="25px"/> ${prev[i].display_name}: ${prev[i].message_body}</li>`;
  }
  // scroll to bottom of message box
  document.querySelector("#msgList").scrollTop =
...
async function delRoom(roomid) {
  console.log(roomid);
  await fetch(`/api/rooms/${roomid}`, { method: "DELETE" }).catch((err) =>
    console.log(err)
  );
  roomList();
}
...
async function onChangeMsgFile(e) {
  e.preventDefault();
  if (e.target.files[0].type == "video/mp4") {    
    ...
    reader.onload = function (evt) {      
      socket.emit(
        "messagefile",
        { roomId: currentRoomId, type: "video", video: evt.target.result },
        async (status) => {
    ...
    reader.onload = function (evt) {
      socket.emit(
        "messagefile",
        { roomId: currentRoomId, type: "image", imageData: evt.target.result },
        async (status) => {
          console.log("status --> ", status);
        }
      );
...
async function sendMsgFile(e) {
  e.preventDefault();
  const msgfile = document.querySelector("#msgfile").value;
  if (msg) {
    socket.emit("messagefile", msgfile, (status) => {
      console.log(status);
    });
    document.querySelector("#msgfile").value = "";
  }
}
...
socket.on("receivefile", (file) => {  
  if (file.type == "video") {
    ...
    document.querySelector(
      "#msgList"
    ).innerHTML += `<li><img src="${file.avatar}" alt="loading.." height="100px" width="100px"/></li>`;
    document.querySelector("#msgList").scrollTop =
      document.querySelector("#msgList").scrollHeight;
  }
});
...
socket.on("receivedMsg", (data) => {
  document.querySelector(
    "#msgList"
  ).innerHTML += `<li><img src="./assets/avatars/${data.avatar}"  alt="avatar" height="25px" width="25px"/> ${data.displayName}: ${data.msg}</li>`;
  document.querySelector("#msgList").scrollTop =
    document.querySelector("#msgList").scrollHeight;
});
...
socket.on("enteredRoom", (data) => {
  document.querySelector(
    "#msgList"
  ).innerHTML += `<li class="system-msg">User ${data.displayName} has entered the room</li>`;
  document.querySelector("#msgList").scrollTop =
    document.querySelector("#msgList").scrollHeight;
  userList();
});
...
socket.on("disconnected", (data) => {
  document.querySelector(
    "#msgList"
  ).innerHTML += `<li class="system-msg">User ${data.displayName} has left the room</li>`;
  document.querySelector("#msgList").scrollTop =
    document.querySelector("#msgList").scrollHeight;
  userList();
});
